...
# Simple ui_schema template
def get_ui_schema(fields_config):
    """Generate a standardized ui_schema"""
    return {
        'sections': {
            'header': {
...
# Define vouchers with simpler structure
VOUCHERS = [
...
for code, name, description, fields in VOUCHERS:
    try:
        ...
        config.ui_schema = get_ui_schema(fields)
        config.name = name
        ...
        config = VoucherModeConfig.objects.create(
            ...
            ui_schema=get_ui_schema(fields),
            is_active=True
        )
