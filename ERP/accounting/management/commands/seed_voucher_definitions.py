...
class Command(BaseCommand):
    help = "Seed baseline voucher definitions per organization."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--reset",
            action="store_true",
            help="Delete existing voucher configs for the org before seeding.",
        )
        parser.add_argument(
            "--repair",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        for org in orgs:
            stats = seed_voucher_configs(org, reset=reset, repair=repair)
            self.stdout.write(
                self.style.SUCCESS(
