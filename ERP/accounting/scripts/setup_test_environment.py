...
def setup_test_environment():
    """
    Sets up the initial data required for testing the journal entry module.
    """
    ...
    organization, org_created = Organization.objects.get_or_create(
        name="TestCorp",
        defaults={'email': 'contact@testcorp.com', 'is_active': True}
    )
    if org_created:
        print(f"Organization '{organization.name}' created.")
    else:
    ...
    if curr_created:
        print(f"Currency '{currency.currency_code}' created.")
    else:
        print(f"Currency '{currency.currency_code}' already exists.")
    ...
    if fy_created:
        print(f"Fiscal Year '{fiscal_year.name}' created.")
    else:
        print(f"Fiscal Year '{fiscal_year.name}' already exists.")
    ...
    if ap_created:
        print(f"Accounting Period '{accounting_period.name}' created.")
    else:
        print(f"Accounting Period '{accounting_period.name}' already exists.")
    ...
    for at_data in account_types_data:
        ...
        if at_created:
            print(f"Account Type '{at.name}' created.")
        else:
            print(f"Account Type '{at.name}' already exists.")
    ...
    for coa_data in chart_of_accounts_data:
        ...
        if coa_created:
            print(f"Chart of Account '{coa.account_name}' created.")
        else:
            print(f"Chart of Account '{coa.account_name}' already exists.")
